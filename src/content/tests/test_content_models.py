...
class TestContentModels:
    ...
    def test_content_block_creation(self):
        content = Content.objects.create(identifier='home', title='Inicio', language='es')
    ...
    def test_content_block_unique_together(self):
        ...
        ContentBlock.objects.create(content=content, identifier='q1', title='Q1', language='es')
        
        # Same content, same identifier, same language - Fail
        with pytest.raises(IntegrityError):
            ContentBlock.objects.create(content=content, identifier='q1', title='Q1 Duplicate', language='es')

    def test_content_cascade_deletion(self):
        content = Content.objects.create(identifier='delete-me', title='Delete', language='es')
